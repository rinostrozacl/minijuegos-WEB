...
export const useChunkedUpload = () => {
  ...
  const hasUnityStructure = (files: File[]): boolean => {
    ...
    const hasIndex = fileNames.some(name => name.toLowerCase().includes('index.html'));
    const hasBuild = fileNames.some(name => name.toLowerCase().includes('build/'));
    return hasIndex && hasBuild;
  };

  // Crear ZIP optimizado para Unity WebGL
  const createOptimizedUnityZip = async (files: File[], onProgress?: (progress: number) => void): Promise<Blob> => {
    const zip = new JSZip();
    ...
    for (let i = 0; i < sortedFiles.length; i++) {
      ...
      const zipPath = relativePath.startsWith('./') ? relativePath.slice(2) : relativePath;
  ...
  const createZipFromFiles = async (files: File[], onProgress?: (progress: number) => void): Promise<Blob> => {
    const zip = new JSZip();
  ...
  const uploadFileInChunks = async (
    ...
    const { themeId, chunkSize = 10 * 1024 * 1024, onProgress, onChunkProgress } = options;
    const uploadId = generateUploadId();
    const totalChunks = Math.ceil(file.size / chunkSize);

    console.log(`[ChunkedUpload] Iniciando upload: ${fileName} (${file.size} bytes, ${totalChunks} chunks)`);
    ...
    for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
      ...
      try {
        ...
        console.log(`[ChunkedUpload] Chunk ${chunkIndex + 1}/${totalChunks} subido`);
        ...
        console.error(`[ChunkedUpload] Error subiendo chunk ${chunkIndex}:`, error);
        throw new Error(`Error subiendo chunk ${chunkIndex + 1}: ${error.message}`);
      }
    }
  ...
  const smartUpload = async (files: File[], themeId: string): Promise<UploadResult> => {
    try {
      ...
      if (hasUnityStructure(files)) {
        ...
        console.log('[SmartUpload] Estructura Unity detectada - creando ZIP optimizado');
